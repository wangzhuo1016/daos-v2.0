...
class IorTestBase(DfuseTestBase):
    ...
    def run_ior(self, manager, processes, intercept=None,  display_space=True,
        ...
        if intercept:
            ...
            self.log.info("HERE intercept")
            if env.get('D_LOG_MASK', None) is None:
                self.log.info("HERE setting D_LOG_MASK=INFO")
                env['D_LOG_MASK'] = 'INFO'
            else:
                self.log.info("HERE D_LOG_MASK={}".format(env['D_LOG_MASK']))
